...
int leer_del_1_al_2 (const string& mensaje){
    string linea;
    while (true){
        cout << mensaje;
        getline(cin, linea);
...
int leer_del_1_4 (const string& mensaje){
    string linea;
    ...
    while (true){
        ...
        if (linea.empty()){
            cout << "Entrada invalida. Ingrese 1 o 4." << "\n";
            continue;
        }
        if (linea.length() != 1){
            cout << "Entrada invalida. Ingrese 1 o 4." << "\n";
...
void editar_articulo(Articulo inventario[], int cantidad_de_articulos, int &x){
    string codigo_a_buscar;
    while (true){
        cout << "ingrese el codigo del articulo a editar: "; cin >> codigo_a_buscar;
        cin.ignore(10000, '\n');
    ...
    for (int i = 0; i < cantidad_de_articulos; i++){
        if (inventario[i].codigo == codigo_a_buscar){
            ...
            cout << "1. Codigo: " << inventario[i].codigo << "\n";
            cout << "2. Nombre: " << inventario[i].nombre << "\n";
            cout << "3. Cantidad minima: " << inventario[i].cantidad_minima << "\n";
            cout << "4. Cantidad maxima: " << inventario[i].cantidad_maxima << "\n";
            cout << "Stock actual: " << inventario[i].stock << "\n";
            x = leer_del_1_4("Seleccione el campo a editar: ");
    ...
    while (true) {
        ...
        switch (opciones){
            ...
            case 5:
                limpiar_pantalla();
                if (cantidad_de_articulos == 0){
                    cout << "No hay articulos para editar o consultar." << "\n" <<  "Cree un articulo primero.";
                    cout << "\n\n";
                    pausar();
                    limpiar_pantalla();
                    pantalla_menu();
                ...
                while (true){
                    cout << "1. Editar articulo" << "\n";
                    cout << "2. Consultar articulo" << "\n";
                    ec = leer_del_1_al_2("Seleccione una opcion (1-2): ");
                    if (ec == 1){
                        while (true){
                            x = verificacion_de_digitos("cuantos articulos quieres editar?: ");
                            if (x == 0){
                                cout << "no se editara ningun articulo." << "\n\n";
                                break;
                            }
                            if (x > cantidad_de_articulos){
                                cout << "No hay tantos articulos para editar. Hay un total de " << cantidad_de_articulos << " articulos." << "\n";
                                continue;
                            }
                            for (int i = 0; i < x; i++){
                                cout << "Editar articulo " << i + 1 << "\n";
